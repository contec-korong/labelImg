...
class CoCoWriter:
    ...
    def add_bnd_box(self, x_min, y_min, width, height, name, difficult):
        bnd_box = {'xmin': x_min, 'ymin': y_min, 'width': width, 'height': height,
    ...
    def append_objects(self, top):
        for each_object in self.box_list:
            ...
            xmax = int(float(each_object['xmin'])) + int(float(each_object['width']))
            ymax = int(float(each_object['ymin'])) + int(float(each_object['height']))
            if ymax == int(float(self.img_size[0])) or (int(float(each_object['ymin'])) == 1):
                truncated.text = "1"  # max == height or min
            elif (xmax == int(float(self.img_size[1]))) or (int(float(each_object['xmin'])) == 1):
            ...
            width = SubElement(bnd_box, 'width')
            width.text = str(each_object['width'])
            height = SubElement(bnd_box, 'height')
...
class CoCoReader:
    ...
    def add_shape(self, label, bnd_box, difficult):
        ...
        x_max = int(float(bnd_box.find('xmin').text)) + int(float(bnd_box.find('width').text))
        y_max = int(float(bnd_box.find('ymin').text)) + int(float(bnd_box.find('height').text))
